...
async def main():
    async with AsyncWebCrawler() as crawler:
        ...
        # 匹配日期和文章链接
        pattern = r'(\b\d{4}年\d{1,2}月\d{1,2}日\b).*?\]\((https://www\.85la\.com/\d+\.html)\)'
        date_url_pairs = re.findall(pattern, result.markdown, re.DOTALL)
        
        latest_date = None
        latest_url = None
        
        for date_str, url in date_url_pairs:
            try:
        ...
        # 2. 输出初始提取结果
        if latest_date and latest_url: # Removed latest_title
            ...
            try:
                ...
                if link_result and hasattr(link_result, 'markdown') and v2ray_match:
                    ...
                    if os.path.exists(file_path):
                        ...
                        print(f"订阅地址已追加到 {file_path}")
                    else:
                        # 文件不存在，新建文件并写入
                        with open(file_path, 'w', encoding='utf-8') as f:
                            f.write(v2ray_content + '\n')
                        print(f"已创建 {file_path} 并添加订阅地址")
                else:
                    print("\n===== 未找到有效的v2ray订阅地址 =====")
                    print("\n未找到V2ray订阅地址")
